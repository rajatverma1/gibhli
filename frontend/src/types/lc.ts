...
export interface LCRules {
  ...
  preAdviceReference?: string;
  courierName?: string;
  confirmationInstruction: ConfirmationInstruction;
  preferredConfirmingBank?: string;
  isTransferable: boolean;
  preferredTransferringBank?: string;
  isRevolving: boolean;
...
export interface GoodsDescription {
  ...
  namedPlace: string;
  countryOfOrigin: string;
  importLicenseType: 'OGL' | 'LICENSE';
  importLicenseRef?: string;
}
...
export interface UploadedDocument {
  id: string;
  name: string;
  type: 'PROFORMA_INVOICE' | 'PURCHASE_ORDER' | 'IMPORT_LICENSE' | 'OTHER';
  filename: string;
  uploadedAt: string;
}
...
export interface PaymentTerms {
  ...
  tenorBasis?: string;
  tenorDays?: number;
  draweeBankType?: string;
  mixedPaymentDetails?: string;
  deferredNegotiationDetails?: string;
  specialPaymentConditions?: string;
}

export interface SettlementCharges {
  debitAccountForPayment: string;
  bankChargesBy: BankChargesCode;
  chargesDebitAccount?: string;
  confirmationChargesBy?: string;
